 ...
 * global: <ID> ("=" ("+" | "-")? <UNSIGNED>)?
 * funcationDefinition: ("void" | "int") <ID> "(" (parameterList)? ")" "{" localDeclarations
 ...
 * nullStatement: ";"
 * compoundStatement: "{" compoundList (returnStatement)? "}"
 * compoundList: statement compoundList | λ
 * readintStatement: "readint" "(" <ID> ")" ";"
 * whileStatement: "while" "(" expr ")" statement
 * ifStatement: "if" "(" expr")" (statement | returnStatement) elsePart
 * elsePart: "else" (statement | returnStatement) | λ
 ...
 * factor: <UNSIGNED> | <ID> | funcationCall | "+" factor | "(" expr ")"
 *	| "-" (<UNSIGNED> | <ID> | funcationCall | ("+")+ ("-" factor | factor) | "-" factor
 *	| "(" expr ")")
 */
import java.util.*;
import java.io.*;
...
class S6
{
	...
	{
		boolean debug = false;
		System.out.println("S6 cpmpiler written by 一只骑着轮子飘在天际的萝莉控抖M羊");
		if(args.length == 2)
		{
			...
			{
				System.out.println("Wrong cmd args");
				System.exit(1);
			}
		}
		...
		outFile.println("; from S6 compiler written by 一只骑着轮子飘在天际的萝莉控抖M羊");
...
class S6Parser implements S6Constants
{
	...
	private void programUnit()
	{
		switch(currentToken.kind)
		{
			case EXTERN:
				...
				break;
			case INT:
				if(getToken(2).kind == ID && getToken(3).kind == LEFTPAREN)
					funcationDefinition();
				else
					globalDeclaration();
				break;
	...
	{
		Token t;
		int retVal = currentToken.kind;
		if(retVal == VOID)
			consume(VOID);
		else if(retVal == INT)
			consume(INT);
		else
			throw genEx("Expecting return value");
		t = currentToken;
		consume(ID);
	...
	private void localDeclarations(int retVal)
	{
		...
		if(retVal == VOID)
			st.enter("return",0,LOCAL);
		else
			st.enter("return",relativeAddress,LOCAL);
	...
	{
		if(currentToken.kind == RIGHTBRACE || currentToken.kind == RETURN)
			return;
		statement();
		statementList();
	}
	...
	private void returnStatement()
	{
		int index,retAdd;
		consume(RETURN);
		index = st.find("return");
		retAdd = st.getRelAdd(index);
		if(retAdd == 0)
		{
			consume(SEMICOLON);
		}
		...
		{
			cg.emitInstruction("cora",retAdd + "");
			expr();
			cg.emitInstruction("stav");
			consume(SEMICOLON);
		}
